...
// Transisi status: disetujui -> dikirim
  ...
  try {
    ...
    if (existing.status !== "disetujui") {
      return res.status(409).json({
        code: 409,
        message: `Transaksi berstatus "${existing.status}" belum bisa dikirim (harus disetujui dulu)`,
      });
    }

    const [row] = await sql`
      UPDATE penjualan SET status = 'dikirim' WHERE id = ${id}
