...
// POST /api/penjualan/:id/approve
// Transisi status: pending -> disetujui
  ...
  try {
    ...
    if (existing.status !== "pending") {
      return res.status(409).json({
        code: 409,
        message: `Transaksi berstatus "${existing.status}" tidak bisa di-approve`,
      });
    }

    const [row] = await sql`
      UPDATE penjualan SET status = 'disetujui' WHERE id = ${id}
